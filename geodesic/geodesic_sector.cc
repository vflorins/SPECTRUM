...
// Set INTR, GHST, and BNDR masks
            if(BoundaryVert_Int(i, j) != -1) RAISE_BITS(vert_mask[vert], GEOELM_BNDR);
            if(IsInteriorVert_Int(i, j)) RAISE_BITS(vert_mask[vert], GEOELM_INTR);
...
// Set INTR, GHST, and BNDR masks
               if(BoundaryEdge_Int(etype, i, j) != -1) RAISE_BITS(edge_mask[edge], GEOELM_BNDR);
               if(IsInteriorEdge_Int(etype, i, j)) RAISE_BITS(edge_mask[edge], GEOELM_INTR);
...
// Set INTR and GHST masks
            if(IsInteriorFace_Int(i, j)) RAISE_BITS(face_mask[face], GEOELM_INTR);

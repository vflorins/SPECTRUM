...
// Pentagonal sectors are no allowed
...
// Record singular corners.
   for(iv = 0; iv < verts_per_face[divs]; iv++) corners[iv] = (NVertNbrs(divs, fv_con[divs][sect][iv]) < edges_per_vert[divs]);

// Find the interior SW corner vertex and t-face.
   int vert_start = fv_con[divs][sect][0];
   it = 0;
   while(!IsInside(divs, sect, divf, vf_con[divf][vert_start][it]) && it < NVertNbrs(divs, vert_start)) it++;
...
This rather complicated function performs a walk through a square region that consists of a sector surrounded by a layer of ghost faces on each side. The walk is performed in the QAS pattern starting from the base (SW) corner, and the index of each t-face visited and each vertex encountered are recorded. NB: The QAS is the addressing systems used by grid blocks.
*/
template <int max_division>

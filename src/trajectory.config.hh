/*!
\file trajectory.config.hh
\brief (Hyper)parameters and config(uration) options for a SPECTRUM trajectory class
\author Lucius Schoenbaum
\date 09/29/2025

This file is part of the SPECTRUM suite of scientific numerical simulation codes. SPECTRUM stands for Space Plasma and Energetic Charged particle TRansport on Unstructured Meshes. The code simulates plasma or neutral particle flows using MHD equations on a grid, transport of cosmic rays using stochastic or grid based methods. The "unstructured" part refers to the use of a geodesic mesh providing a uniform coverage of the surface of a sphere.
*/

/*
 * This file is automatically generated by config.py. Do not edit this file, instead edit config.py. 
 */
 
#ifndef SPECTRUM_TRAJECTORY_CONFIG_HH
#define SPECTRUM_TRAJECTORY_CONFIG_HH

#include "common/compiletime_lists.hh"
#include "trajectory.config.fields.hh"

namespace Spectrum {

template<Config::Trajectory trajectory_, SpecieId specieid_>
struct TrajectoryConfig;

/*!
\brief (Hyper)parameters and config(uration) options for a SPECTRUM Trajectory class
\author Lucius Schoenbaum
\date 09/29/2025
Trajectory: Fieldline
*/
template<SpecieId specieid_>
struct TrajectoryConfig<Config::Trajectory::Fieldline, specieid_> {
   static constexpr Specie<specieid_> specie;
   using Coordinates = TrajectoryCoordinates<Config::Trajectory::Fieldline, specieid_>;
   using Fields = TrajectoryFields<Config::Trajectory::Fieldline, specieid_>;
   using RecordCoordinates = Coordinates;
   using FieldlineField_t = Mag_t;
//! Name: time_flow
// Description: The time flow direction used for simulation
// Options: forward | backward
   static constexpr auto time_flow = TrajectoryOptions::TimeFlow::forward;
//! Name: rk_integrator
// Description: The discretization scheme used for simulation, a Runge-Kutta scheme
// Options: Euler1E | Euler1I | Midpoint_2I | Midpoint_2E | RungeKutta_4E | Kutta38_4E | RungeKuttaFehlberg_54E | CashKarp_54E | DormandPrince_54E | RungeKuttaFehlberg65E | RungeKuttaFehlberg76E | RungeKuttaFehlberg_87E | ...others (see source)
   static constexpr auto rk_integrator = RKIntegrator::DormandPrince_54E;
//! Name: record_mag_extrema
// Description: Whether to record magnetic field extrema
   static constexpr auto record_mag_extrema = false;
//! Name: record_trajectory
// Description: Whether to record segments
   static constexpr auto record_trajectory = false;
//! Name: record_trajectory_segment_presize
// Description: Default initial size of trajectory segment record
   static constexpr auto record_trajectory_segment_presize = 10000;
//! Name: advance_safety_level
// Description: Trajectory advance routine safety level
// Options: low: no checks | medium: check dt only | high: check dt, number of segments, and time adaptations per step
   static constexpr auto advance_safety_level = TrajectoryOptions::SafetyLevel::low;
//! Name: max_trajectory_steps
// Description: Largest length for single trajectory
   static constexpr auto max_trajectory_steps = 100000;
//! Name: max_time_adaptations
// Description: Largest number of time step adaptations for a single time step
   static constexpr auto max_time_adaptations = 1;
//! Name: n_max_calls
// Description: Upper limit on the number of steps in debug mode
// Options: -1: unlimited | n ≥ 0: limited, upper bound n
   static constexpr auto n_max_calls = -1;
};


/*!
\brief (Hyper)parameters and config(uration) options for a SPECTRUM Trajectory class
\author Lucius Schoenbaum
\date 09/29/2025
Trajectory: Focused
*/
template<SpecieId specieid_>
struct TrajectoryConfig<Config::Trajectory::Focused, specieid_> {
   static constexpr Specie<specieid_> specie;
   using Coordinates = TrajectoryCoordinates<Config::Trajectory::Focused, specieid_>;
   using Fields = TrajectoryFields<Config::Trajectory::Focused, specieid_>;
   using RecordCoordinates = Coordinates;
//! Name: time_flow
// Description: The time flow direction used for simulation
// Options: forward | backward
   static constexpr auto time_flow = TrajectoryOptions::TimeFlow::forward;
//! Name: rk_integrator
// Description: The discretization scheme used for simulation, a Runge-Kutta scheme
// Options: Euler1E | Euler1I | Midpoint_2I | Midpoint_2E | RungeKutta_4E | Kutta38_4E | RungeKuttaFehlberg_54E | CashKarp_54E | DormandPrince_54E | RungeKuttaFehlberg65E | RungeKuttaFehlberg76E | RungeKuttaFehlberg_87E | ...others (see source)
   static constexpr auto rk_integrator = RKIntegrator::DormandPrince_54E;
//! Name: record_mag_extrema
// Description: Whether to record magnetic field extrema
   static constexpr auto record_mag_extrema = false;
//! Name: record_trajectory
// Description: Whether to record segments
   static constexpr auto record_trajectory = false;
//! Name: record_trajectory_segment_presize
// Description: Default initial size of trajectory segment record
   static constexpr auto record_trajectory_segment_presize = 10000;
//! Name: advance_safety_level
// Description: Trajectory advance routine safety level
// Options: low: no checks | medium: check dt only | high: check dt, number of segments, and time adaptations per step
   static constexpr auto advance_safety_level = TrajectoryOptions::SafetyLevel::low;
//! Name: max_trajectory_steps
// Description: Largest length for single trajectory
   static constexpr auto max_trajectory_steps = 100000;
//! Name: max_time_adaptations
// Description: Largest number of time step adaptations for a single time step
   static constexpr auto max_time_adaptations = 1;
//! Name: n_max_calls
// Description: Upper limit on the number of steps in debug mode
// Options: -1: unlimited | n ≥ 0: limited, upper bound n
   static constexpr auto n_max_calls = -1;
//! Name: pperp_method
// Description: Switch controlling how to calculate mu. updating mu according to the scheme does not guarantee conservation of magnetic moment, but can be used with non-adiabatic terms.
// Options: moment_cons: compute mu from magnetic moment conservation | scheme: update according to scheme
   static constexpr auto pperp_method = TrajectoryOptions::PPerpMethod::scheme;
//! Name: use_B_drifts
// Description: Flag to use gradient and curvature drifts in drift velocity calculation
   static constexpr auto use_B_drifts = TrajectoryOptions::UseBDrifts::none;
//! Name: cfl_advection
// Description: CFL condition for advection
   static constexpr auto cfl_advection = 0.5;
//! Name: drift_safety
// Description: Safety factor for drift-based time step (to modify "drift_vel" with a small fraction of the particle's velocity)
   static constexpr auto drift_safety = 0.5;
//! Name: mirror_threshold
// Description: How many time steps to allow before recording a mirror event
   static constexpr auto mirror_threshold = 10;
};


/*!
\brief (Hyper)parameters and config(uration) options for a SPECTRUM Trajectory class
\author Lucius Schoenbaum
\date 09/29/2025
Trajectory: Guiding
*/
template<SpecieId specieid_>
struct TrajectoryConfig<Config::Trajectory::Guiding, specieid_> {
   static constexpr Specie<specieid_> specie;
   using Coordinates = TrajectoryCoordinates<Config::Trajectory::Guiding, specieid_>;
   using Fields = TrajectoryFields<Config::Trajectory::Guiding, specieid_>;
   using RecordCoordinates = Coordinates;
//! Name: time_flow
// Description: The time flow direction used for simulation
// Options: forward | backward
   static constexpr auto time_flow = TrajectoryOptions::TimeFlow::forward;
//! Name: rk_integrator
// Description: The discretization scheme used for simulation, a Runge-Kutta scheme
// Options: Euler1E | Euler1I | Midpoint_2I | Midpoint_2E | RungeKutta_4E | Kutta38_4E | RungeKuttaFehlberg_54E | CashKarp_54E | DormandPrince_54E | RungeKuttaFehlberg65E | RungeKuttaFehlberg76E | RungeKuttaFehlberg_87E | ...others (see source)
   static constexpr auto rk_integrator = RKIntegrator::DormandPrince_54E;
//! Name: record_mag_extrema
// Description: Whether to record magnetic field extrema
   static constexpr auto record_mag_extrema = false;
//! Name: record_trajectory
// Description: Whether to record segments
   static constexpr auto record_trajectory = false;
//! Name: record_trajectory_segment_presize
// Description: Default initial size of trajectory segment record
   static constexpr auto record_trajectory_segment_presize = 10000;
//! Name: advance_safety_level
// Description: Trajectory advance routine safety level
// Options: low: no checks | medium: check dt only | high: check dt, number of segments, and time adaptations per step
   static constexpr auto advance_safety_level = TrajectoryOptions::SafetyLevel::low;
//! Name: max_trajectory_steps
// Description: Largest length for single trajectory
   static constexpr auto max_trajectory_steps = 100000;
//! Name: max_time_adaptations
// Description: Largest number of time step adaptations for a single time step
   static constexpr auto max_time_adaptations = 1;
//! Name: n_max_calls
// Description: Upper limit on the number of steps in debug mode
// Options: -1: unlimited | n ≥ 0: limited, upper bound n
   static constexpr auto n_max_calls = -1;
//! Name: pperp_method
// Description: Switch controlling how to calculate mu. updating mu according to the scheme does not guarantee conservation of magnetic moment, but can be used with non-adiabatic terms.
// Options: moment_cons: compute mu from magnetic moment conservation | scheme: update according to scheme
   static constexpr auto pperp_method = TrajectoryOptions::PPerpMethod::scheme;
//! Name: cfl_advection
// Description: CFL condition for advection
   static constexpr auto cfl_advection = 0.5;
//! Name: drift_safety
// Description: Safety factor for drift-based time step (to modify "drift_vel" with a small fraction of the particle's velocity)
   static constexpr auto drift_safety = 0.5;
//! Name: mirror_threshold
// Description: How many time steps to allow before recording a mirror event
   static constexpr auto mirror_threshold = 10;
};


/*!
\brief (Hyper)parameters and config(uration) options for a SPECTRUM Trajectory class
\author Lucius Schoenbaum
\date 09/29/2025
Trajectory: GuidingDiff
*/
template<SpecieId specieid_>
struct TrajectoryConfig<Config::Trajectory::GuidingDiff, specieid_> {
   static constexpr Specie<specieid_> specie;
   using Coordinates = TrajectoryCoordinates<Config::Trajectory::GuidingDiff, specieid_>;
   using Fields = TrajectoryFields<Config::Trajectory::GuidingDiff, specieid_>;
   using RecordCoordinates = Coordinates;
//! Name: time_flow
// Description: The time flow direction used for simulation
// Options: forward | backward
   static constexpr auto time_flow = TrajectoryOptions::TimeFlow::forward;
//! Name: rk_integrator
// Description: The discretization scheme used for simulation, a Runge-Kutta scheme
// Options: Euler1E | Euler1I | Midpoint_2I | Midpoint_2E | RungeKutta_4E | Kutta38_4E | RungeKuttaFehlberg_54E | CashKarp_54E | DormandPrince_54E | RungeKuttaFehlberg65E | RungeKuttaFehlberg76E | RungeKuttaFehlberg_87E | ...others (see source)
   static constexpr auto rk_integrator = RKIntegrator::DormandPrince_54E;
//! Name: record_mag_extrema
// Description: Whether to record magnetic field extrema
   static constexpr auto record_mag_extrema = false;
//! Name: record_trajectory
// Description: Whether to record segments
   static constexpr auto record_trajectory = false;
//! Name: record_trajectory_segment_presize
// Description: Default initial size of trajectory segment record
   static constexpr auto record_trajectory_segment_presize = 10000;
//! Name: advance_safety_level
// Description: Trajectory advance routine safety level
// Options: low: no checks | medium: check dt only | high: check dt, number of segments, and time adaptations per step
   static constexpr auto advance_safety_level = TrajectoryOptions::SafetyLevel::low;
//! Name: max_trajectory_steps
// Description: Largest length for single trajectory
   static constexpr auto max_trajectory_steps = 100000;
//! Name: max_time_adaptations
// Description: Largest number of time step adaptations for a single time step
   static constexpr auto max_time_adaptations = 1;
//! Name: n_max_calls
// Description: Upper limit on the number of steps in debug mode
// Options: -1: unlimited | n ≥ 0: limited, upper bound n
   static constexpr auto n_max_calls = -1;
//! Name: pperp_method
// Description: Switch controlling how to calculate mu. updating mu according to the scheme does not guarantee conservation of magnetic moment, but can be used with non-adiabatic terms.
// Options: moment_cons: compute mu from magnetic moment conservation | scheme: update according to scheme
   static constexpr auto pperp_method = TrajectoryOptions::PPerpMethod::scheme;
//! Name: cfl_advection
// Description: CFL condition for advection
   static constexpr auto cfl_advection = 0.5;
//! Name: drift_safety
// Description: Safety factor for drift-based time step (to modify "drift_vel" with a small fraction of the particle's velocity)
   static constexpr auto drift_safety = 0.5;
//! Name: mirror_threshold
// Description: How many time steps to allow before recording a mirror event
   static constexpr auto mirror_threshold = 10;
};


/*!
\brief (Hyper)parameters and config(uration) options for a SPECTRUM Trajectory class
\author Lucius Schoenbaum
\date 09/29/2025
Trajectory: GuidingScatt
*/
template<SpecieId specieid_>
struct TrajectoryConfig<Config::Trajectory::GuidingScatt, specieid_> {
   static constexpr Specie<specieid_> specie;
   using Coordinates = TrajectoryCoordinates<Config::Trajectory::GuidingScatt, specieid_>;
   using Fields = TrajectoryFields<Config::Trajectory::GuidingScatt, specieid_>;
   using RecordCoordinates = Coordinates;
//! Name: time_flow
// Description: The time flow direction used for simulation
// Options: forward | backward
   static constexpr auto time_flow = TrajectoryOptions::TimeFlow::forward;
//! Name: rk_integrator
// Description: The discretization scheme used for simulation, a Runge-Kutta scheme
// Options: Euler1E | Euler1I | Midpoint_2I | Midpoint_2E | RungeKutta_4E | Kutta38_4E | RungeKuttaFehlberg_54E | CashKarp_54E | DormandPrince_54E | RungeKuttaFehlberg65E | RungeKuttaFehlberg76E | RungeKuttaFehlberg_87E | ...others (see source)
   static constexpr auto rk_integrator = RKIntegrator::DormandPrince_54E;
//! Name: record_mag_extrema
// Description: Whether to record magnetic field extrema
   static constexpr auto record_mag_extrema = false;
//! Name: record_trajectory
// Description: Whether to record segments
   static constexpr auto record_trajectory = false;
//! Name: record_trajectory_segment_presize
// Description: Default initial size of trajectory segment record
   static constexpr auto record_trajectory_segment_presize = 10000;
//! Name: advance_safety_level
// Description: Trajectory advance routine safety level
// Options: low: no checks | medium: check dt only | high: check dt, number of segments, and time adaptations per step
   static constexpr auto advance_safety_level = TrajectoryOptions::SafetyLevel::low;
//! Name: max_trajectory_steps
// Description: Largest length for single trajectory
   static constexpr auto max_trajectory_steps = 100000;
//! Name: max_time_adaptations
// Description: Largest number of time step adaptations for a single time step
   static constexpr auto max_time_adaptations = 1;
//! Name: n_max_calls
// Description: Upper limit on the number of steps in debug mode
// Options: -1: unlimited | n ≥ 0: limited, upper bound n
   static constexpr auto n_max_calls = -1;
//! Name: pperp_method
// Description: Switch controlling how to calculate mu. updating mu according to the scheme does not guarantee conservation of magnetic moment, but can be used with non-adiabatic terms.
// Options: moment_cons: compute mu from magnetic moment conservation | scheme: update according to scheme
   static constexpr auto pperp_method = TrajectoryOptions::PPerpMethod::scheme;
//! Name: cfl_advection
// Description: CFL condition for advection
   static constexpr auto cfl_advection = 0.5;
//! Name: drift_safety
// Description: Safety factor for drift-based time step (to modify "drift_vel" with a small fraction of the particle's velocity)
   static constexpr auto drift_safety = 0.5;
//! Name: mirror_threshold
// Description: How many time steps to allow before recording a mirror event
   static constexpr auto mirror_threshold = 10;
//! Name: split_scatt_fraction
// Description: Whether to split the diffusive advance into two (one before and one after the advection).
// Options: 0.0: do not split | >0.0: fraction of stochastic step to take before deterministic step
   static constexpr auto split_scatt_fraction = 0.0;
//! Name: const_dmumax
// Description: Desired accuracy in pitch angle cosine or in pitch angle mu
// Options: constant_dtheta_max: dtheta_max = 2π/180 (deg to rad conversion factor) | constant_dmumax: dmumax = 0.02 (desired accuracy in pitch angle cosine)
   static constexpr auto const_dmumax = TrajectoryOptions::ConstDmumax::constant_dtheta_max;
//! Name: stochastic_method
// Description: Which stochastic method to use for scattering
// Options: Euler | Milstein | RK2
   static constexpr auto stochastic_method = TrajectoryOptions::StochasticMethod::Euler;
//! Name: cfl_pitchangle
// Description: CFL condition for pitch angle scattering
   static constexpr auto cfl_pitchangle = 0.5;
};


/*!
\brief (Hyper)parameters and config(uration) options for a SPECTRUM Trajectory class
\author Lucius Schoenbaum
\date 09/29/2025
Trajectory: GuidingDiffScatt
*/
template<SpecieId specieid_>
struct TrajectoryConfig<Config::Trajectory::GuidingDiffScatt, specieid_> {
   static constexpr Specie<specieid_> specie;
   using Coordinates = TrajectoryCoordinates<Config::Trajectory::GuidingDiffScatt, specieid_>;
   using Fields = TrajectoryFields<Config::Trajectory::GuidingDiffScatt, specieid_>;
   using RecordCoordinates = Coordinates;
//! Name: time_flow
// Description: The time flow direction used for simulation
// Options: forward | backward
   static constexpr auto time_flow = TrajectoryOptions::TimeFlow::forward;
//! Name: rk_integrator
// Description: The discretization scheme used for simulation, a Runge-Kutta scheme
// Options: Euler1E | Euler1I | Midpoint_2I | Midpoint_2E | RungeKutta_4E | Kutta38_4E | RungeKuttaFehlberg_54E | CashKarp_54E | DormandPrince_54E | RungeKuttaFehlberg65E | RungeKuttaFehlberg76E | RungeKuttaFehlberg_87E | ...others (see source)
   static constexpr auto rk_integrator = RKIntegrator::DormandPrince_54E;
//! Name: record_mag_extrema
// Description: Whether to record magnetic field extrema
   static constexpr auto record_mag_extrema = false;
//! Name: record_trajectory
// Description: Whether to record segments
   static constexpr auto record_trajectory = false;
//! Name: record_trajectory_segment_presize
// Description: Default initial size of trajectory segment record
   static constexpr auto record_trajectory_segment_presize = 10000;
//! Name: advance_safety_level
// Description: Trajectory advance routine safety level
// Options: low: no checks | medium: check dt only | high: check dt, number of segments, and time adaptations per step
   static constexpr auto advance_safety_level = TrajectoryOptions::SafetyLevel::low;
//! Name: max_trajectory_steps
// Description: Largest length for single trajectory
   static constexpr auto max_trajectory_steps = 100000;
//! Name: max_time_adaptations
// Description: Largest number of time step adaptations for a single time step
   static constexpr auto max_time_adaptations = 1;
//! Name: n_max_calls
// Description: Upper limit on the number of steps in debug mode
// Options: -1: unlimited | n ≥ 0: limited, upper bound n
   static constexpr auto n_max_calls = -1;
//! Name: pperp_method
// Description: Switch controlling how to calculate mu. updating mu according to the scheme does not guarantee conservation of magnetic moment, but can be used with non-adiabatic terms.
// Options: moment_cons: compute mu from magnetic moment conservation | scheme: update according to scheme
   static constexpr auto pperp_method = TrajectoryOptions::PPerpMethod::scheme;
//! Name: cfl_advection
// Description: CFL condition for advection
   static constexpr auto cfl_advection = 0.5;
//! Name: drift_safety
// Description: Safety factor for drift-based time step (to modify "drift_vel" with a small fraction of the particle's velocity)
   static constexpr auto drift_safety = 0.5;
//! Name: mirror_threshold
// Description: How many time steps to allow before recording a mirror event
   static constexpr auto mirror_threshold = 10;
//! Name: split_scatt_fraction
// Description: Whether to split the diffusive advance into two (one before and one after the advection).
// Options: 0.0: do not split | >0.0: fraction of stochastic step to take before deterministic step
   static constexpr auto split_scatt_fraction = 0.0;
//! Name: const_dmumax
// Description: Desired accuracy in pitch angle cosine or in pitch angle mu
// Options: constant_dtheta_max: dtheta_max = 2π/180 (deg to rad conversion factor) | constant_dmumax: dmumax = 0.02 (desired accuracy in pitch angle cosine)
   static constexpr auto const_dmumax = TrajectoryOptions::ConstDmumax::constant_dtheta_max;
//! Name: stochastic_method_mu
// Description: which stochastic method to use for pitch angle scattering
// Options: Euler | Milstein | RK2
   static constexpr auto stochastic_method_mu = TrajectoryOptions::StochasticMethod::Euler;
//! Name: stochastic_method_perp
// Description: Which stochastic method to use for perpendicular diffusion
// Options: Euler | Milstein | RK2
   static constexpr auto stochastic_method_perp = TrajectoryOptions::StochasticMethod::Euler;
//! Name: cfl_pitchangle
// Description: CFL condition for pitch angle scattering
   static constexpr auto cfl_pitchangle = 0.5;
};


/*!
\brief (Hyper)parameters and config(uration) options for a SPECTRUM Trajectory class
\author Lucius Schoenbaum
\date 09/29/2025
Trajectory: Lorentz
*/
template<SpecieId specieid_>
struct TrajectoryConfig<Config::Trajectory::Lorentz, specieid_> {
   static constexpr Specie<specieid_> specie;
   using Coordinates = TrajectoryCoordinates<Config::Trajectory::Lorentz, specieid_>;
   using Fields = TrajectoryFields<Config::Trajectory::Lorentz, specieid_>;
   using RecordCoordinates = Coordinates;
//! Name: time_flow
// Description: The time flow direction used for simulation
// Options: forward | backward
   static constexpr auto time_flow = TrajectoryOptions::TimeFlow::forward;
//! Name: rk_integrator
// Description: The discretization scheme used for simulation, a Runge-Kutta scheme
// Options: Euler1E | Euler1I | Midpoint_2I | Midpoint_2E | RungeKutta_4E | Kutta38_4E | RungeKuttaFehlberg_54E | CashKarp_54E | DormandPrince_54E | RungeKuttaFehlberg65E | RungeKuttaFehlberg76E | RungeKuttaFehlberg_87E | ...others (see source)
   static constexpr auto rk_integrator = RKIntegrator::DormandPrince_54E;
//! Name: record_mag_extrema
// Description: Whether to record magnetic field extrema
   static constexpr auto record_mag_extrema = false;
//! Name: record_trajectory
// Description: Whether to record segments
   static constexpr auto record_trajectory = false;
//! Name: record_trajectory_segment_presize
// Description: Default initial size of trajectory segment record
   static constexpr auto record_trajectory_segment_presize = 10000;
//! Name: advance_safety_level
// Description: Trajectory advance routine safety level
// Options: low: no checks | medium: check dt only | high: check dt, number of segments, and time adaptations per step
   static constexpr auto advance_safety_level = TrajectoryOptions::SafetyLevel::low;
//! Name: max_trajectory_steps
// Description: Largest length for single trajectory
   static constexpr auto max_trajectory_steps = 100000;
//! Name: max_time_adaptations
// Description: Largest number of time step adaptations for a single time step
   static constexpr auto max_time_adaptations = 1;
//! Name: n_max_calls
// Description: Upper limit on the number of steps in debug mode
// Options: -1: unlimited | n ≥ 0: limited, upper bound n
   static constexpr auto n_max_calls = -1;
//! Name: pperp_method
// Description: Switch controlling how to calculate mu. updating mu according to the scheme does not guarantee conservation of magnetic moment, but can be used with non-adiabatic terms.
// Options: moment_cons: compute mu from magnetic moment conservation | scheme: update according to scheme
   static constexpr auto pperp_method = TrajectoryOptions::PPerpMethod::scheme;
//! Name: cfl_advection
// Description: CFL condition for advection
   static constexpr auto cfl_advection = 0.1;
//! Name: drift_safety
// Description: Safety factor for drift-based time step (to modify "drift_vel" with a small fraction of the particle's velocity)
   static constexpr auto drift_safety = 0.5;
//! Name: mirror_threshold
// Description: How many time steps to allow before recording a mirror event
   static constexpr auto mirror_threshold = 300;
//! Name: steps_per_orbit
// Description: Number of time steps per one orbit
   static constexpr auto steps_per_orbit = 100;
};


/*!
\brief (Hyper)parameters and config(uration) options for a SPECTRUM Trajectory class
\author Lucius Schoenbaum
\date 09/29/2025
Trajectory: Parker
*/
template<SpecieId specieid_>
struct TrajectoryConfig<Config::Trajectory::Parker, specieid_> {
   static constexpr Specie<specieid_> specie;
   using Coordinates = TrajectoryCoordinates<Config::Trajectory::Parker, specieid_>;
   using Fields = TrajectoryFields<Config::Trajectory::Parker, specieid_>;
   using RecordCoordinates = Coordinates;
//! Name: time_flow
// Description: The time flow direction used for simulation
// Options: forward | backward
   static constexpr auto time_flow = TrajectoryOptions::TimeFlow::forward;
//! Name: rk_integrator
// Description: The discretization scheme used for simulation, a Runge-Kutta scheme
// Options: Euler1E | Euler1I | Midpoint_2I | Midpoint_2E | RungeKutta_4E | Kutta38_4E | RungeKuttaFehlberg_54E | CashKarp_54E | DormandPrince_54E | RungeKuttaFehlberg65E | RungeKuttaFehlberg76E | RungeKuttaFehlberg_87E | ...others (see source)
   static constexpr auto rk_integrator = RKIntegrator::DormandPrince_54E;
//! Name: record_mag_extrema
// Description: Whether to record magnetic field extrema
   static constexpr auto record_mag_extrema = false;
//! Name: record_trajectory
// Description: Whether to record segments
   static constexpr auto record_trajectory = false;
//! Name: record_trajectory_segment_presize
// Description: Default initial size of trajectory segment record
   static constexpr auto record_trajectory_segment_presize = 10000;
//! Name: advance_safety_level
// Description: Trajectory advance routine safety level
// Options: low: no checks | medium: check dt only | high: check dt, number of segments, and time adaptations per step
   static constexpr auto advance_safety_level = TrajectoryOptions::SafetyLevel::low;
//! Name: max_trajectory_steps
// Description: Largest length for single trajectory
   static constexpr auto max_trajectory_steps = 100000;
//! Name: max_time_adaptations
// Description: Largest number of time step adaptations for a single time step
   static constexpr auto max_time_adaptations = 1;
//! Name: n_max_calls
// Description: Upper limit on the number of steps in debug mode
// Options: -1: unlimited | n ≥ 0: limited, upper bound n
   static constexpr auto n_max_calls = -1;
//! Name: pperp_method
// Description: Switch controlling how to calculate mu. updating mu according to the scheme does not guarantee conservation of magnetic moment, but can be used with non-adiabatic terms.
// Options: moment_cons: compute mu from magnetic moment conservation | scheme: update according to scheme
   static constexpr auto pperp_method = TrajectoryOptions::PPerpMethod::scheme;
//! Name: cfl_advection
// Description: CFL condition for advection
   static constexpr auto cfl_advection = 0.5;
//! Name: drift_safety
// Description: Safety factor for drift-based time step (to modify "drift_vel" with a small fraction of the particle's velocity)
   static constexpr auto drift_safety = 0.5;
//! Name: mirror_threshold
// Description: How many time steps to allow before recording a mirror event
   static constexpr auto mirror_threshold = 10;
//! Name: stochastic_method
// Description: Which stochastic method to use for scattering
// Options: Euler | Milstein | RK2
   static constexpr auto stochastic_method = TrajectoryOptions::StochasticMethod::Euler;
//! Name: use_B_drifts
// Description: Flag to use gradient and curvature drifts in drift velocity calculation
   static constexpr auto use_B_drifts = TrajectoryOptions::UseBDrifts::none;
//! Name: divk_method
// Description: Which method of computation to use for divK
// Options: direct: using direct central finite differences | gradients: using background-computed gradient quantities
   static constexpr auto divk_method = TrajectoryOptions::DivkMethod::direct;
//! Name: cfl_diffusion
// Description: CFL condition for diffusion
   static constexpr auto cfl_diffusion = 0.5;
//! Name: cfl_acceleration
// Description: CFL condition for acceleration
   static constexpr auto cfl_acceleration = 0.5;
//! Name: dlnp_max
// Description: Maximum allowed fraction of momentum change per step
   static constexpr auto dlnp_max = 0.01;
};



}

#endif

